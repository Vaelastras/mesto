...
const closeEditButton = document.querySelector('.popup__close'); // ищем кнопку закрытия (попап 1)
const closePlaceButton = document.querySelector('.popup-newplace__close'); // ищем кнопку закрытия (попап 2)
const closeImageButton = document.querySelector('.popup-image__close'); // ищем кнопку закрытия (попап 3)
...
const popupNamePhoto = document.querySelector(".popup-image__title")// ищем название места (попап 3)
...
function renderCard(elementname, elementlink) {
  const card = elementTemplate.cloneNode(true); // копируем шаблон
  ...
  cardTitle.textContent = elementname; // присваиваем текст на параметр функции
  cardImage.src = elementlink; // присваиваем сслку на параметр
  cardImage.setAttribute('alt', cardTitle); // устанавливаем аттрибут альт для картинки с названием нейма
...
/* вешаем листернер на событие клик и приставаем изображению  */
  cardImage.addEventListener("click", item => {
    popupImage.src = item.link;
    popupNamePhoto.textContent = item.name;
    openCloseImagePopup();
  });
